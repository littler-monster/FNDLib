...
class TextCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear1 = nn.Sequential(nn.Linear(81, 100), nn.Linear(100, self.label_num))
    ...
    def forward(self, x, x_c):
        ...
        # print("x3.shape:{}".format(x3.shape))  # x1.shape:torch.Size([64, 1, 27, 1])
        ...
        # print("x.shape:{}".format(x.shape))  # x.shape:torch.Size([64, 1, 27, 3])
        x = x.view(batch, 1, -1)
        # print("x.shape:{}".format(x.shape)) # x.shape:torch.Size([64, 1, 81])
        x = torch.squeeze(x)
