...
class Body_Embedding(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_size_gru, hidden_size_att):
        super(Body_Embedding, self).__init__()

        self.gru1 = nn.GRU(embedding_dim, hidden_size_gru, bidirectional=True, batch_first=True)  # 100,50  用于词编码
        # gru默认情况下的输出格式(seq_length, batch_size, input_size) 使用batch_first=True 后将batch_size当成第一维参数
        self.att1 = SelfAttention(hidden_size_gru * 2, hidden_size_att)  # 100, 100
        self.gru2 = nn.GRU(hidden_size_att, hidden_size_gru, bidirectional=True, batch_first=True)  # 100, 50  用于句编码

    def forward(self, x):
        # x的形状(batch_size, seq_length, embedding_dim)
        x = x.split(1, dim=1)  # seq_length个(batch_size, 1, embedding_dim)
...
class Comment_Embedding(nn.Module):
    def __init__(self, embedding_dim, hidden_size_gru, hidden_size_att):
        ...
        self.gru1 = nn.GRU(embedding_dim, hidden_size_gru, bidirectional=True, batch_first=True)  # 100,50
        self.att1 = SelfAttention(hidden_size_gru * 2, hidden_size_att)  # 100, 100
...
class dEFEND(nn.Module):
    def __init__(self, config):
        ...
        self.Body = Body_Embedding(self.embedding_dim, self.hidden_size_gru, self.hidden_size_att)
        self.Comments = Comment_Embedding(self.embedding_dim, self.hidden_size_gru, self.hidden_size_att)
