...
# 确保 'Weibograph' 目录存在
output_dir = os.path.join(cwd, 'data/Twitter15graph')
...
def main():
    treePath = os.path.join(cwd, 'data/Twitter15/data.TD_RvNN.vol_5000.txt')
    ...
    labelPath = os.path.join(cwd, "data/Twitter15/Twitter15_label_All.txt")
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez( os.path.join(cwd, 'data/Twitter15graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
