import os
from data.dataset import GraphDataset_Bi,BiGraphDataset,UdGraphDataset,GraphDataset_GACL,test_GraphDataset_GACL
cwd=os.getcwd()
...
def loadData(dataname, treeDic,fold_x_train,fold_x_test,droprate):
    ...
    traindata_list = GraphDataset_Bi(fold_x_train, treeDic, droprate=droprate,data_path= data_path)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = GraphDataset_Bi(fold_x_test, treeDic,data_path= data_path)
...
def loadData_BiGCN(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate):
    data_path = os.path.join(cwd,'data', dataname + 'graph')
    print("loading train set", )
...
############################  GACL  #############################################
def loadData_GACL(dataname, fold_x_train,fold_x_test,droprate):
    print("loading train set", )
    traindata_list = GraphDataset_GACL(fold_x_train, droprate=droprate)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = test_GraphDataset_GACL(fold_x_test, droprate=0)
    print("test no:", len(testdata_list))
    return traindata_list, testdata_list
